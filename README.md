...
| 2 | 모호한 상태에서 직접 API 호출 | `endpoint_recall@1`, `slot_fill_accuracy` |
| 3a | 멀티턴 명확화 후 API 호출 | `final_endpoint_hit`, `slot_fill_accuracy`, `avg clarification turns` |
...
- `endpoint_recall@1`: gold endpoint를 정확히 맞췄는지
...
- Stage 2 baseline endpoint recall@1: `0.6222`
- Stage 2 baseline slot accuracy: `0.4019`
- Stage 3 final endpoint hit: `0.7556`
...
- Stage 2보다 Stage 3a의 final endpoint hit / slot accuracy가 높으면 명확화 대화가 실제로 도움이 됨
