...
| 2 | 모호한 상태에서 직접 API 호출 | `match_score`, `any_match` |
| 3a | 멀티턴 명확화 후 API 호출 | `MRR`, `Recall@N`, `avg clarification turns` |
...
- 성공 기준: 예측한 API가 `gold_api.candidate_apis` 중 하나와 일치
- 출력 파일: `results/ambiguity_framework/baseline_<model>.jsonl`

주요 지표:

- `any_match`: 후보 API 중 하나라도 일치했는지
- `match_score`: API/endpoint 일치 정도
...
명확화로 확장된 대화를 다시 CheckEval에 넣어 모호성이 실제로 줄었는지 봅니다.
...
현재 저장된 예시 결과는 아래와 같습니다.

### `gpt-5.4`

- Stage 1 평균 모호성: `0.5539`
- Stage 2 baseline score: `0.0667`
- Stage 3 MRR: `0.3148`
- Stage 3b post ambiguity: `0.3887`
- 평균 모호성 감소: `0.1703`

### `gpt-5.4-mini`

- Stage 1 평균 모호성: `0.5539`
- Stage 2 baseline score: `0.0889`
- Stage 3 MRR: `0.3630`
- Stage 3b post ambiguity: `0.3995`
- 평균 모호성 감소: `0.1565`

해석 기준은 단순합니다.

- `pre_ambiguity`가 높을수록 baseline 성능이 낮아지면 CheckEval 신호가 유효
- Stage 2보다 Stage 3a가 높으면 명확화 대화가 실제로 도움이 됨
